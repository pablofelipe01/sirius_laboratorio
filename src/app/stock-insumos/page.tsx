        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
            ...
            <div className="bg-white/95 backdrop-blur-sm rounded-xl p-6 border border-white/20">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-gray-600 text-sm font-medium">Con Nombre</p>
                  <p className="text-3xl font-bold text-green-600">
                    {insumos.filter(insumo => insumo.fields.Item && insumo.fields.Item.trim()).length}
                  </p>
                </div>
            ...
            <div className="bg-white/95 backdrop-blur-sm rounded-xl p-6 border border-white/20">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-gray-600 text-sm font-medium">Sin Configurar</p>
            ...
            <div className="bg-white/95 backdrop-blur-sm rounded-xl p-6 border border-white/20">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-3xl font-bold text-purple-600">{entradas.length}</p>
                </div>
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="border-b-2 border-gray-200">
                        <th className="text-left py-4 px-4 font-semibold text-gray-700">Nombre del Insumo</th>
                        <th className="text-center py-4 px-4 font-semibold text-gray-700">Unidad</th>
                        <th className="text-center py-4 px-4 font-semibold text-gray-700">Entradas</th>
                        <th className="text-center py-4 px-4 font-semibold text-gray-700">Salidas</th>
                    ...
                    <tbody>
                      {insumosFiltrados.map((insumo, index) => {
                        const numEntradas = insumo.fields['Entrada Insumos']?.length || 0;
                        const numSalidas = insumo.fields['Salida Insumos']?.length || 0;
                        const hasName = insumo.fields.Item && insumo.fields.Item.trim();
                        
                        return (
                          <tr key={insumo.id} className={`border-b border-gray-100 hover:bg-gray-50 transition-colors ${index % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'}`}>
                            <td className="py-4 px-4">
                              <div className="flex items-center gap-2">
                                {!hasName && <span className="text-orange-500">⚠️</span>}
                                <div>
                                  <div className="font-medium text-gray-900">
                                    {hasName ? insumo.fields.Item : `Sin nombre - ID: ${insumo.id.slice(-6)}`}
                                  </div>
                                  {!hasName && (
                                    <div className="text-xs text-orange-600">Necesita configuración</div>
                                  )}
                                </div>
                              </div>
                            </td>
                            ...
                            <td className="py-4 px-4 text-center">
                              <span className={`font-bold ${numEntradas > 0 ? 'text-green-600' : 'text-gray-400'}`}>
                                {numEntradas}
                              </span>
                            </td>
                            <td className="py-4 px-4 text-center">
                              <span className={`font-bold ${numSalidas > 0 ? 'text-red-600' : 'text-gray-400'}`}>
                                {numSalidas}
                              </span>
                            </td>
                ...
                {selectedInsumo && (
                  <div className="bg-gray-50 p-4 rounded-lg">
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      {(!selectedInsumo.fields.Item || !selectedInsumo.fields.Item.trim()) && (
                        <span className="text-orange-500">⚠️</span>
                      )}
                      <p className="text-gray-700">
                        {selectedInsumo.fields.Item && selectedInsumo.fields.Item.trim() 
                          ? selectedInsumo.fields.Item 
                          : `Sin nombre - ID: ${selectedInsumo.id.slice(-6)}`
                        }
                      </p>
                    </div>
                    <p className="text-sm text-gray-500 mt-1">
                      Unidad: {selectedInsumo.fields.Unidades?.value || 'Sin unidad'}
                    </p>
                    {(!selectedInsumo.fields.Item || !selectedInsumo.fields.Item.trim()) && (
                      <p className="text-xs text-orange-600 mt-2">
                        💡 Tip: Este insumo necesita un nombre descriptivo en Airtable
                      </p>
                    )}
                  </div>
                )}
